...
import { resolvePageMeta } from "@/components/site-header-meta"
...
function orgBreadcrumbLabel(
  ...
  if (!liveMode) return "Acme Corp"
  return operatorContext?.org_name ?? "Organization"
}
...
function HeaderSearch({ search }: Readonly<{ search: string }>) {
  return (
    <>
      <button
        type="button"
...
function TimeRangeSlot({ liveMode }: Readonly<{ liveMode: boolean }>) {
  if (liveMode) {
    return (
      <Button
        variant="outline"
...
function NotificationsMenu({ liveMode }: Readonly<{ liveMode: boolean }>) {
  return (
    <DropdownMenu>
...
function liveAccountAria(operatorContext: OperatorContext | null): string {
  if (!operatorContext) return "Operator context unavailable"
  return `Operator role ${operatorContext.role ?? "unmapped"} in ${operatorContext.org_name}`
}

function LiveAccountButton({
  operatorContext,
  ...
  return (
    <Button
      ...
      aria-label={liveAccountAria(operatorContext)}
    >
      ...
      <DropdownMenuContent align="end" className="w-72 rounded-xl p-1.5">
        <div className="flex items-center gap-3 rounded-lg bg-muted/50 px-2.5 py-2.5">
          <Avatar className="size-10 rounded-full ring-1 ring-border">
            <AvatarImage src="/avatars/shadcn.jpg" alt="Operator" />
            <AvatarFallback className="rounded-full text-[12px]">OP</AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <div className="truncate text-[13px] font-medium">Operator</div>
            <div className="truncate text-[12px] text-muted-foreground">you@acme.com</div>
...
function AccountControl({
  ...
  return liveMode ? (
    <LiveAccountButton operatorContext={operatorContext} />
  ) : (
    <PreviewAccountMenu />
  )
}
...
function HeaderRightActions({
  ...
  return (
    <div className="flex flex-1 items-center justify-end gap-1">
      ...
      <AccountControl liveMode={liveMode} operatorContext={operatorContext} />
    </div>
  )
}
    ...
    <header className="flex h-(--header-height) shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height)">
      <div className="flex w-full items-center gap-2 px-4 lg:px-6">
        <HeaderBreadcrumb
          ...
          meta={meta}
        />
        <HeaderSearch search={meta.search} />
        <HeaderRightActions
          liveMode={liveMode}
          operatorContext={operatorContext}
