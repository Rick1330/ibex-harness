...
func TestUnit_TokenListToProto_identity(t *testing.T) {
	...
	m := out[0]
	if m.GetTokenId() != row.ID {
		t.Fatalf("token_id=%q want %q", m.GetTokenId(), row.ID)
	}
	if m.GetName() != row.Name {
		t.Fatalf("name=%q want %q", m.GetName(), row.Name)
	}
	if m.GetPrefix() != row.Prefix {
		t.Fatalf("prefix=%q want %q", m.GetPrefix(), row.Prefix)
	}
	if m.GetPermissions() != row.Permissions {
		t.Fatalf("permissions=%d want %d", m.GetPermissions(), row.Permissions)
	}
}
...
func TestUnit_TokenListToProto_omitsNilTimestamps(t *testing.T) {
	t.Parallel()
	row := service.TokenListItem{
		ID: "tid", Name: "n", Prefix: "p", Permissions: 1,
		CreatedAt: time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC),
	}
	m := tokenListToProto([]service.TokenListItem{row})[0]

	if m.ExpiresAt != nil {
		t.Fatalf("ExpiresAt set: %v", m.ExpiresAt)
	}
	if m.RevokedAt != nil {
		t.Fatalf("RevokedAt set: %v", m.RevokedAt)
	}
}
