...
(digit runs, short tokens resembling entities) and on time-shaped words (weekly,
evening). Each string below avoids person names, email/phone patterns, location
phrases, and DATE_TIME triggers; verified clean in CI via
test_http_fixture_strings_presidio_clean.
"""
...
HTTP_SEED_CONTENT = (
    "Organized folders improve retrieval speed during structured workspace audits"
)

# Distinct bodies per HTTP integration scenario (no shared content_hash collisions).
HTTP_NOVEL_WRITE_CONTENT = (
    "Dark theme reduces glare for dashboard review sessions"
)
...
# Fixed body for broken-redis after-commit resilience test (distinct from fixtures above).
HTTP_BROKEN_REDIS_WRITE_CONTENT = (
    "After commit resilience probe uses neutral operational wording only"
)
