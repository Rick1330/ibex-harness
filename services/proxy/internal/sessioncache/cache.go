...
func (c *Cache) Set(ctx context.Context, k LookupKey, e Entry) {
	...
	_ = c.client.Set(ctx, Key(k), payload, c.ttl).Err() //nolint:errcheck // fail-open: cache miss is acceptable; session will be fetched from Postgres
}
...
func (c *Cache) Invalidate(ctx context.Context, k LookupKey) {
	...
	_ = c.client.Del(ctx, Key(k)).Err() //nolint:errcheck // fail-open: cache miss is acceptable; session will be fetched from Postgres
}
