"""Best-effort Redis publish for budget/rate-card invalidation (4.P.4)."""
...
_CHANNEL_PREFIX = "budget_updates:"
_EVENT_VERSION = 1
_REDIS_SOCKET_TIMEOUT_SECONDS = 1.0


class BudgetPublisher(Protocol):
    async def publish_budget_update(self, org_id: str) -> None: ...


class NoopBudgetPublisher:
    async def publish_budget_update(self, org_id: str) -> None:
        del org_id
        await asyncio.sleep(0)


class RecordingBudgetPublisher:
...
class RedisBudgetPublisher:
    """PUBLISH {v, org_id} to budget_updates:{org_id}."""
