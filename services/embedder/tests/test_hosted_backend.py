...
from app.errors import EmptyBatchError, GeometryMismatchError, InvalidVectorError
...
def _make_backend(
    *,
    ...
    if embed_side_effect is not None:
        client.embed = AsyncMock(side_effect=embed_side_effect)
    else:
        vecs = embed_return if embed_return is not None else _raw_vectors(1, dimensions)
...
class TestConstruction:
    ...
    def test_name_is_provider(self) -> None:
        backend = _make_backend(provider="cohere", model_id="embed-english-v3.0")
...
class TestEmbed:
    async def test_l2_normalizes_openai_raw_vectors(self) -> None:
        ...
        backend = _make_backend(embed_return=raw)
    ...
    async def test_wrong_dim_raises(self) -> None:
        backend = _make_backend(dimensions=_DIM, embed_return=_raw_vectors(1, 4))
    ...
    async def test_zero_vector_rejected(self) -> None:
        zeros = np.zeros((1, _DIM), dtype=np.float32)
        backend = _make_backend(embed_return=zeros)
        with pytest.raises(InvalidVectorError):
            await backend.embed(["x"])
