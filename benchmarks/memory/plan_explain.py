...
- GIN runtime verification uses an RLS-off probe query with planner hints
  (`explain_gin_probe_plan` + `assert_gin_index_used`) in `test_gin_index_used_at_runtime`;
...
async def prepare_gin_probe_session(session: AsyncSession) -> None:
    ...
    Service-account RLS bypass lets the planner satisfy @@ via btree bitmap scans on
    idx_memories_agent_active without touching the GIN counter; disable RLS locally and
    discourage seq/bitmap/parallel plans so EXPLAIN/pg_stat reflect GIN access.
    """
    await session.execute(
        text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text.avoid-sqlalchemy-text
