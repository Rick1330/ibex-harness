...
merge_scoped_profiles() {
  ...
  boot_subset="$(grep -vE "$MODE_PREFIX" "$boot_profile" | grep 'responsepipeline\.go' || true)"
  http_subset="$(grep -vE "$MODE_PREFIX" "$http_profile" | grep 'chat_provider\.go' | filter_http_chat_provider_lines || true)"
  ...
  {
    grep -E "$MODE_PREFIX" "$rp_profile"
    grep -vE "$MODE_PREFIX" "$rp_profile" || true
    printf '%s\n' "$boot_subset"
    printf '%s\n' "$http_subset"
  } >"$filtered_out"
  return 0
