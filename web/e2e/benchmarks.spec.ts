...
test.describe("benchmark dashboard", () => {
  ...
  test("navigates Memory latency, history, and run detail", async ({ page }) => {
    await page.goto("/benchmarks/memory/latency");
    await expect(page).toHaveURL(/\/benchmarks\/memory\/latency$/);
    await expect(page.url()).not.toMatch(/\.txt$/);
    await expect(page.getByRole("heading", { name: "Memory latency" })).toBeVisible();

    await page.goto("/benchmarks/memory/history");
    await expect(page).toHaveURL(/\/benchmarks\/memory\/history$/);
    await expect(page.getByRole("heading", { name: "Memory history" })).toBeVisible();

    await page.goto("/benchmarks/memory/history/0");
    await expect(page).toHaveURL(/\/benchmarks\/memory\/history\/0$/);
    await expect(page.getByRole("heading", { name: "Memory run 0" })).toBeVisible();
