...
<Callout type="success" title="Phase 3.5 complete">
  Exit audit signed off ([phase35-exit-audit](/roadmap/phase-3-5-extraction-assembly/phase35-exit-audit)).
  Detailed gap register: [033-phase3.5-exit-audit](/roadmap/archive/033-phase3.5-exit-audit).
  Decision log: [decisions](/roadmap/phase-3-5-extraction-assembly/decisions).
</Callout>
...
See the [decision log](/roadmap/phase-3-5-extraction-assembly/decisions) for the ADR register.

## Phase 3.5 Exit Criteria

1. Extraction worker converts a completed session into correctly-categorized memories — **met** (F.1)
2. A subsequent chat request transparently injects a relevant memory — **met** (F.1)
3. Zero cross-tenant memory leakage across all ISO-\* and ISO-MCP-\* cases — **met** (F.2 + Phase 3 ISO-\*)
4. Context assembly p95 &lt; 50ms at 20 concurrent requests; proxy overhead stays &lt;20ms p99 non-provider time — **partial** (local harness + proxy microbench; dedicated suites deferred — see exit audit)
5. All 4 degradation-ladder levels independently verified — **met** (C.6 + F.1 L3)
6. MCP `search_memory`/`write_memory`/`record_feedback` tools function end-to-end, sharing one substrate with the proxy — **met** (E.2–E.3)
7. MCP server fails closed on auth-service outage; independent rate-limit budget verified — **met** (E.4)
8. All Phase 1, Phase 2, and Phase 3 regression tests still pass — **met** (required CI gates)
9. `make e2e-smoke-p3.5` exits 0 — **met** (F.1 CI)
10. ADR-0038, ADR-0050, and Phase 3.5 ADRs 0062–0072 published and indexed (planning aliases ADR-0043/0044 are tokenizer / response-pipeline — not reused) — **met** (F.3)
