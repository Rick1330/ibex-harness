...
### Post-response async (bounded)

```go
// After response is written to client — submit to bounded pool/queue (see Backpressure).
// Do NOT spawn unbounded go func() per request.
// WithoutCancel keeps request-scoped logger/trace values; timeout detaches cancel.
...
Same contract as [2.4.3](/roadmap/phase-2-single-provider/milestones/2.4.3-proxy-session-lifecycle): bound post-response work with a worker pool or drop-oldest queue. Register via `packages/metrics`:
...
- [x] Trace record in ClickHouse for every completed LLM request within 500ms of response
- [x] Trace includes all fields from the schema (no nil/zero fields for non-optional data)
- [x] Trace write failure does NOT affect LLM response (logged at WARN, not propagated)
- [x] No LLM content (prompt, completion text) in trace record
- [x] `proxy_overhead_ms` (total_latency_ms) in trace matches measured proxy overhead
- [x] Async emission uses bounded pool/queue; `ibex_proxy_async_queue_depth` and `ibex_proxy_async_dropped_total` metrics present

---

## Implementation notes (complete)

- `assembleTrace` + post-response `enqueuePostResponse` on the existing non-dropping checkpoint `asyncpool`
- Success (stream/non-stream), incomplete stream, and provider-failure emit paths
- Auth/directive stage latencies on context; `checkpoint_id` left nil (store does not return IDs yet)
- Optional when `CLICKHOUSE_DSN` unset (`TraceWriter` nil)