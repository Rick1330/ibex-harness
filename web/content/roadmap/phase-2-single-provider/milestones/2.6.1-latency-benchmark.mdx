...
**ADR required:** [ADR-0034](/docs/adr/0034-performance-methodology) — Performance measurement methodology
...
- [x] **Hard prerequisite:** Synthetic stages in `benchmarks/go/proxy_overhead_test.go` are replaced with real package calls (`authcache`, `ratelimit`, directive resolve, prompt inject) before this gate is green — placeholder crypto/string stages do **not** count
- [x] Benchmark runs with a mock provider (no real OpenAI calls) through the **full** middleware chain
- [x] k6 defaults to `GET /health` (CI smoke/fast); `K6_USE_CHAT=1` on the **full** profile for `POST /v1/chat/completions`
- [x] p99 proxy overhead < 20ms at 100 concurrent requests (k6 full profile + Go parallel chat bench)
- [x] Stage breakdown metrics exported and visible (`ibex_proxy_auth_duration_seconds`, `ibex_proxy_provider_duration_seconds`, existing request/directive histograms)
- [x] Regression baseline committed: pin `target_commit` / `baseline_sha` in `benchmarks/data-schema/baseline.json` after the first **real** (non-synthetic) run
- [x] CI fails if p99 exceeds baseline by more than 20%
- [x] ADR-0034 written defining "proxy overhead" precisely
