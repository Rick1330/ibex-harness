...
**ADR required:** ADR-0026 — OpenAI client design and retry policy
...
// Directive injection is deferred to milestone 2.3.3; see ADR-0025.
...
// PassthroughFields merge permitted client fields; model/messages/stream cannot be overridden.
...
- **`TestOpenAIClient_DirectiveInjection`** *(deferred to [milestone 2.3.3](/roadmap/phase-2-single-provider/milestones/2.3.3-system-prompt-injection))*: request with `SystemDirective` set → first message in outgoing body has `role="system"` with directive content
...
- [x] `packages/provider/openai.Client` implements `packages/provider.Provider`
- [x] Non-streaming requests return correct provider response
- [ ] Directive is injected as first system message when set (deferred to milestone 2.3.3)
- [x] Retry policy retries on 429/5xx with exponential backoff + jitter
- [x] No retry on 4xx client errors
- [x] API key is never logged (enforced by test)
- [x] Connection pooling: single shared `http.Client`, not one per request
- [x] All provider errors mapped to IBEX error codes before returning to handler
