...
`feature/m2-5-2-clickhouse-client`

## PR Title

`feat(clickhouse): typed batch writer for llm_traces (m2.5.2)`
...
- Default batch size: 500 rows (`CLICKHOUSE_INSERT_BATCH_SIZE`)
- Default flush interval: 200ms (`CLICKHOUSE_INSERT_FLUSH_MS`) — supports Goal 2.5 ≤500ms visibility
...
- [x] `Write` is non-blocking; returns before the insert completes
- [x] Batch flush occurs when size reaches MaxBatchSize OR FlushInterval elapses
- [x] `Close` flushes remaining rows before returning
- [x] Flush error does NOT propagate to callers of `Write` (best-effort delivery)
- [x] Writer registered with `packages/shutdown.Coordinator` in proxy `main.go`

---

## Next

[2.5.3 Async trace emitter](/roadmap/phase-2-single-provider/milestones/2.5.3-async-trace-emitter)
