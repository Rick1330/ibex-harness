...
The async post-response processing is done via a **bounded worker pool or drop-oldest queue** (not unbounded `go func()` per request). It has its own context with a 5-second deadline (not the request context, which is cancelled when the response is sent).

### Backpressure (required)

Unbounded post-response goroutines will OOM under load. Bound async work:

- Worker pool with fixed concurrency **or** a fixed-capacity drop-oldest queue
- Emit metrics via `packages/metrics`:
  - `ibex_proxy_async_queue_depth` — current queue depth
  - `ibex_proxy_async_dropped_total` — tasks dropped when the queue is full
...
- [ ] Post-response work is bounded (worker pool or drop-oldest queue); metrics `ibex_proxy_async_queue_depth` and `ibex_proxy_async_dropped_total` registered via `packages/metrics`
