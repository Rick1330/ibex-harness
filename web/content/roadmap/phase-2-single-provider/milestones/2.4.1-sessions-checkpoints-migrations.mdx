...
**ADR:** [ADR-0032 — Session data model and retention strategy](/docs/adr/0032-session-data-model)
...
- [x] RLS enforced on both tables (`FORCE` + `rls_org_visible`, matching directives)
- [x] `checkpoints` has `UNIQUE(session_id, turn_index)` — no duplicate turns
- [x] `idx_sessions_agent_extraction` index supports Phase 3 worker queries
- [x] ADR-0032 written covering session lifecycle, retention policy, and Phase 3 extraction model

> **Note:** Shipped SQL hardens the draft snippet above: composite tenant FKs, append-only checkpoint grants/trigger, partial unique on `external_id`, and shared `rls_org_visible` instead of inline `current_setting` policies. See [ADR-0032](/docs/adr/0032-session-data-model).
