...
```go
// ChatParseMiddleware parses/validates the body and attaches llm.ChatCompletionRequest.
...
// MustProviderFromContext retrieves the selected provider from ctx.
// Panics if called outside a handler that ran ProviderRoutingMiddleware.
func MustProviderFromContext(ctx context.Context) provider.Provider
```
...
(DirectiveResolve is not implemented in this milestone; ChatParse sits immediately before ProviderRouter today.)
...
- **`TestUnit_MustProviderFromContext_PanicsWithout`**: panic when middleware skipped
