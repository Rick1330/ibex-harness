...
**ADR required:** ADR-0027 — Streaming response strategy and dual-write design
...
Streaming in the proxy creates a unique technical challenge: the proxy must simultaneously **forward** bytes to the caller (for low TTFB) and **accumulate** them best-effort (for later session/trace hooks). This is the "dual-write" design. Both writes must be correct:
- Forward must be byte-exact (SSE format must be preserved; clients parse it directly)
- Accumulation is best-effort under a 1 MiB UTF-8-safe soft cap (full persistence is milestones 2.4/2.5)
...
## ADR-0027 — Streaming strategy

Write `web/content/docs/adr/0027-streaming-dual-write.mdx` covering:
...
```go
// StreamAccumulator collects completion text best-effort (1 MiB UTF-8-safe soft cap)
// without blocking the stream forward path.
// Safe for concurrent Write (TeeReader) and post-stream Wait (later 2.4/2.5 hooks).
...
- **`TestUnit_Streaming_ForwardsChunksInOrder`**: mock provider sends 5 SSE chunks; client receives them in order with flush-per-event
- **`TestUnit_StreamAccumulator_Complete`**: accumulator holds the concatenated content after `[DONE]`
- **`TestUnit_StreamAccumulator_DoneSignalReached`**: `[DONE]` unblocks `StreamAccumulator.Wait()` (channel sync; no sleep-before-write)
- **`TestUnit_Streaming_ProviderErrorMidStream`**: provider `CloseWithError` after 3 chunks → partial stream, no `[DONE]`, `Complete()==false`
- **`TestUnit_Streaming_ClientDisconnect`**: hang stub signals entry via channel, then cancel; handler returns and writer WaitGroup joins (no goroutine leak)

---

## Acceptance Criteria

- [x] Streaming completions forward SSE bytes to client in real time (flush after each event)
- [x] `X-Accel-Buffering: no` header set to prevent Nginx buffering
- [x] `StreamAccumulator` collects content best-effort without blocking the forward path (1 MiB UTF-8-safe soft cap)
- [x] Provider error mid-stream results in partial stream + incomplete flag (not a server crash); trace emission is 2.4/2.5
- [x] Client disconnect cancels context; proxy stops forwarding and cleans up (no goroutine leak)
- [x] ADR-0027 written and indexed
