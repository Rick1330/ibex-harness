...
**ADR:** [ADR-0031 — System prompt injection strategy](/docs/adr/0031-system-prompt-injection)
...
## ADR-0031 — Prompt injection strategy
...
- [x] `system_first` mode: directive is first message; `role="system"`; original messages follow
- [x] `system_append` mode: directive is inserted after the leading contiguous system block; subsequent messages retain their order
- [x] `user_prepend` mode: directive is prepended to the first user message content
- [x] Empty directive: messages returned unchanged
- [x] Input slice is never mutated (pure function — safe for concurrent use)
- [x] Injection is tested with realistic GPT-4o message arrays including multi-turn history
