...
description: "Phase 3 is the largest and most complex phase. A formal exit gate ensures nothing was shipped partially and that the system is ready for Phase 4 (multi-provider) — fingerprints/billing live in Phase 6."
summary: "Phase 3 is the largest and most complex phase. A formal exit gate ensures nothing was shipped partially and that the system is ready for Phase 4 (multi-provider) — fingerprints/billing live in Phase 6."
status: "planned"
milestoneId: "3.9.3"
...
Phase 3 is the largest and most complex phase. A formal exit gate ensures nothing was shipped partially and that the system is ready for Phase 4 (multi-provider support and routing). Behavioural fingerprinting, drift, and billing are **Phase 6** — not Phase 4 readiness.
...
- [ ] `web/engineering/DEVELOPMENT_GUIDE.md` updated with Phase 3 local setup (6 new services)
- [ ] All ADRs (0032–0044) written and indexed in `web/engineering/adr/README.md`
...
API="${IBEX_API_ADDR:-http://localhost:8081}"
...
for i in $(seq 1 30); do
    MEMORY_COUNT=$(curl -sf "$API/v1/agents/$AGENT/memories" \
...
- [ ] All Phase 3 ADRs indexed and linked from the architecture doc

## Phase 6 readiness (not Phase 4)

Fingerprints, drift, and billing are **out of scope for Phase 4**. Track under [Phase 6](/roadmap/phase-6-intelligence-monetization):

- [ ] `ibex_core.behavioral_fingerprints` schema drafted (ADR + migration when Phase 6 starts)
- [ ] Drift alert pipeline and billing integrity dashboards deferred until Phase 6 goals are expanded
