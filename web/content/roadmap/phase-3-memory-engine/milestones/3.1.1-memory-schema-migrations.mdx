...
- **Evolvability:** The Phase 3 schema is the base; Phase 6 adds behavioural fingerprinting and drift metrics on top
...
- `ibex_core.behavioral_fingerprints` (Phase 6 — Intelligence & Monetization)
- `ibex_core.drift_alerts` (Phase 6 — Intelligence & Monetization)
- Memory compression or summarisation (Phase 4 or later; not fingerprints)
- pgvector HNSW index (requires pgvector 0.5+; planned when any org exceeds ~1M memories — typically Phase 4 scale work, independent of fingerprints)
...
- **Why IVFFlat over HNSW:** IVFFlat is available in pgvector 0.4+; HNSW requires 0.5+. IVFFlat is sufficient for < 5M vectors per org. HNSW is planned for Phase 4 scale work when any org exceeds 1M memories (independent of Phase 6 fingerprints).
...
- **Why `expires_at` is nullable:** Not all memories expire. Operator-created memories (user_provided source) may be permanent. Extracted memories default to null (no expiry in Phase 3; decay scoring may land with Phase 6 intelligence work).
