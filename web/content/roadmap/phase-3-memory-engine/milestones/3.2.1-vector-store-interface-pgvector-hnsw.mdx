...
**Status:** Complete (merged via #600 — embed client, HNSW benches, ADR-0053)  
**Goal:** [Track B — Vector Store & Embedding](/roadmap/phase-3-memory-engine/tracks#track-b--vector-store--embedding)  
...
**ADR:** [ADR-0053](/docs/adr/0053-vector-store-abstraction) — Vector store abstraction and composite scoring v2 (planning sketches said ADR-0041; that number is the model capability registry)
...
<Callout type="info" title="Planning baseline (sketch deviations)">
  Shipped reality vs this sketch: flat `services/memory/app/` (not `src/ibex_memory/`);
  ADR-**0053** (not ADR-0041); benches under `benchmarks/memory/` with published JSON at
  `web/public/benchmarks/hnsw-benchmark-data.json`; search SQL uses a CTE (HNSW `ORDER BY … LIMIT`
  then outer `min_similarity` filter); defaults stay `ef_search=40` and `min_similarity=0.70`
  with `iterative_scan` off; **1M HNSW benches are CI-only** (local = 10K/100K). Large backfills
  should load-then-`CREATE INDEX` (see [DEPLOYMENT.md](/engineering/DEPLOYMENT.md)).
</Callout>
...
Authoritative record: [`0053-vector-store-abstraction.mdx`](/docs/adr/0053-vector-store-abstraction). Planning excerpt (orientation only):
...
- [x] `VectorStore` ABC + `PgVectorStore` implementation with `ef_search` set per-transaction (PR-A/B)
- [x] Composite scoring v2 implemented with category-conditional half-life, unit-tested against the documented formula (PR-A)
- [x] Recall/latency benchmark run at 10K/100K synthetic rows under fixed methodology (TRUNCATE + ANALYZE + EXPLAIN/pg_stat gates); results under `benchmarks/memory/output/` + published `web/public/benchmarks/hnsw-benchmark-data.json` (PR-C). **1M is CI-only** (`Memory Benchmarks` workflow profile `full`)
- [x] Embedder HTTP client with zero direct ML library imports — Semgrep `ibex-memory-no-ml-imports` (PR-C)
- [x] [ADR-0053](/docs/adr/0053-vector-store-abstraction) published (defaults: `ef_search=40`, `min_similarity=0.70`, `iterative_scan` off)
