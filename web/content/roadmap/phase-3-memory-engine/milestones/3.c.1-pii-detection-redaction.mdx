...
- [ ] Named-entity PII (person names, addresses) flagged with confidence score; findings below threshold route to `quarantined` status, not silently redacted
- [ ] Redaction happens strictly before the embedding stage — enforced by pipeline ordering test, not just convention
- [ ] Unit tests cover both true positives and known false-positive traps
- [ ] `pii_detected` and `pii_redacted` flags set correctly on the `memories` row
