...
**Status:** In progress (`feature/m3-c-2-dedup`)  
**Goal:** [Track C — Write Pipeline](/roadmap/phase-3-memory-engine/tracks#track-c--write-pipeline)  
**Phase:** 3 — Core Memory Substrate  
**Estimated effort:** 2 days  
**ADR:** [ADR-0055](/docs/adr/0055-memory-write-path-dedup)
...
- [x] Identical `content_hash` → no insert path, `retrieval_count` incremented on existing row, no LLM call fired
- [x] Near-duplicate candidates (cosine > 0.92) fetched via `VectorStore.search` (Track B) — no duplicate ANN SQL
- [x] Near-dup detection is org/agent-scoped (no cross-tenant leakage in candidate search)
- [x] Empty corpus case handled correctly (new agent, no prior memories)
- [x] Partial unique index on active `(org_id, agent_id, content_hash)` (`000018`)

<Callout type="info" title="Planning baseline (sketch deviations)">
  Shipped vs sketch: use live `VectorStore.search` (no `find_similar` ABC method);
  candidates keep `similarity > threshold` after a `>= threshold` store query; hash is
  computed on **post-PII** content; pipeline order validate → pii → exact_dedup → embed →
  near_dedup (ADR-0055).
</Callout>
