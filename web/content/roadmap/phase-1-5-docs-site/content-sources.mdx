...
description: "Tracks the current public docs corpus against implementation-native sources and highlights where late Phase 2 coverage still needs rebaselining."
summary: "Tracks the current public docs corpus against implementation-native sources and highlights where late Phase 2 coverage still needs rebaselining."
...
| --- | --- | --- | --- |
| Getting started | `web/content/roadmap/current-state.mdx`, `Makefile`, `services/auth/README.md`, `services/proxy/README.md` | Needs rebaseline | Several pages still teach Phase 1 `501` behavior rather than current mock/live Phase 2 behavior |
| Architecture | `services/README.md`, `services/*/README.md`, accepted ADRs | Needs rebaseline | Public pages still understate forwarding, sessions, and ClickHouse traces |
| Proxy | `services/proxy/README.md`, `services/proxy/internal/http/`, `packages/directive`, `packages/injection`, `packages/authcache`, `packages/idempotency`, `packages/session` | Highest priority | This is where most late-Phase-2 drift currently lives |
| Auth | `services/auth/README.md`, `packages/proto/proto/ibex/auth/v1/auth.proto`, accepted ADRs | Mostly accurate | Public auth pages need lighter updates than proxy pages |
| Deployment / ops | `services/*/.env.example`, `infra/compose/dev/.env.example`, `services/proxy/internal/config/`, `packages/clickhouse`, `packages/metrics`, `packages/telemetry` | Needs expansion | Env/config docs still reflect an earlier runtime surface |
| API reference | Manual MDX + proto contracts + route inventory | Mixed | Keep manual behavioral pages; defer OpenAPI until a maintained spec exists |

## Getting Started

| Public page | Source | Notes |
| --- | --- | --- |
| Introduction | `/roadmap/current-state`, `services/README.md` | Rebaseline to current late-Phase-2 capability boundary |
| Concepts | `services/README.md`, `services/proxy/README.md`, `web/engineering/GLOSSARY.md` | Distinguish current proxy features from planned Phase 3 memory systems |
| Quickstart | `Makefile`, `compose-dev`, `services/auth/README.md`, `services/proxy/README.md` | Happy path should document `mock` mode success, with `live` mode as optional |
| FAQ | Current docs corpus + issue/PR feedback | Expand as real late-Phase-2 operator questions appear |
