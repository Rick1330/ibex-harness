---
title: "IBEX Harness"
description: "> Single source of truth for designing, building and shipping > `ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *docs-site* related. > The landing page is already built in a separate repository > (this Lovable project) and is referenced here only at the boundar"
summary: "> Single source of truth for designing, building and shipping > `ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *docs-site* related. > The landing page is already built in a separate repository > (this Lovable project) and is referenced here only at the boundar"
...
- Landing site remains on Vercel (`ibexdepo`); docs are a separate Worker.
...
**Deliverables**
- `pnpm-workspace.yaml` with `web` and `packages/*` and `packages/*`.
...
4. **OG cards** — both sites must produce valid OG cards. Visiting
   `ibexharness.com` and `ibexharness.com` in a Slack unfurl
