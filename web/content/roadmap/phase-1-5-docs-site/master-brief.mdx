---
title: "IBEX Harness"
description: "> Single source of truth for designing, building and shipping > `ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *public web* related. > Part 0 documents the unified monorepo pivot; historical Part I boundary text is retained below for context."
summary: "> Single source of truth for designing, building and shipping > `ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *public web* related. > Part 0 documents the unified monorepo pivot; historical Part I boundary text is retained below for context."
phase: "phase-1-5-docs-site"
fullTitle: "IBEX Harness — Docs Site Master Brief"
---

# IBEX Harness — Docs Site Master Brief

<Callout type="warn" title="Visual system superseded (2026-07, #279)">
  Typography, color, and motion guidance in this brief (Geist, monochrome accent, no Framer Motion) is **superseded** by [SITE_DESIGN_SYSTEM.md](/engineering/SITE_DESIGN_SYSTEM) — Paper/Ink OKLCH tokens, Instrument Serif / Inter / JetBrains Mono, Ember accent, and reduced-motion-aware Framer Motion.
</Callout>


<Callout type="note" title="Part 0 — Architecture pivot (2026-07)">
  **Supersedes Part I boundary contract.** The public product ships as a **unified**
  Next.js static export from `web/` at `https://ibexharness.com` (landing `/`,
  docs `/docs`, benchmarks, blog, releases, roadmap). Deploy:
  `.github/workflows/web-deploy.yml` → Cloudflare Pages. Legacy
  `docs.ibexharness.com` redirects to apex. Paths in this document that reference
  `docs/app/` mean `web/` in the current repo layout.
</Callout>

> **Single source of truth** for designing, building and shipping
> `ibexharness.com`. This document supersedes
> `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *public web* related.
>
> <Callout type="note" title="Superseded — Part I">
>   Part I below assumed a separate landing repository (Lovable/Vercel). Landing
>   now lives in `web/src/app/page.tsx` in this monorepo. Keep Part I for audit
>   history only.
> </Callout>
...
(`AGENTS.md` / docs-site design brief). It applies to every chat in this repo.
...
# Operating Mindset
You are building ibexharness.com — a developer-tools documentation
...
- All new components live under `web/src/components/` and are
  exported by name (no default exports except for Next.js route files).
- MDX components are registered in `web/src/mdx-components.tsx`
...
(`ibexharness.com`) and the docs site (`ibexharness.com`) so a
...
The docs site lives at `web/`. The Go proxy, auth service,
...
### C.3 Hosting: **Cloudflare Workers** (amended 2026-06-22)

> **Original brief:** Vercel project `ibex-harness-docs`. **Superseded** by Cloudflare Workers + GitHub Actions deploy — see [ADR-0023](/docs/adr/0023-docs-site-architecture) and `ibex-harness-workspace/ops/cloudflare/README.md` (local-only operator runbook).

- [@opennextjs/cloudflare](https://opennext.js.org/cloudflare) bundles Next.js 15 for Workers.
- Deploy only via `.github/workflows/web-deploy.yml` (not Cloudflare Workers Builds Git).
- Preview validation: `web-build` CI on PRs + optional `workflow_dispatch` deploy smoke test.
- Landing, docs, benchmarks, blog, and releases ship from one Next.js static export in `web/` at `ibexharness.com`; legacy `docs.ibexharness.com` 301s to apex paths.
...
- [ ] `pnpm docs:dev` is a valid command (errors are OK at this stage
      because `web` doesn't exist yet — but the command must
      resolve to `turbo`).
...
`web/package.json` — dependencies pinned per §C.2.
...
`web/src/app/docs/[[...slug]]/page.tsx` and `layout.tsx` — use
...
`web/src/app/globals.css` — the full token file. Use the
...
export const metadata = {
  metadataBase: new URL("https://ibexharness.com/docs"),
...
`web/src/app/layout.config.tsx` — wordmark nav per §A.5.
...
**Deliverables** — every component in `web/src/components/mdx/`:
...
`web/src/app/docs/[[...slug]]/opengraph-image.tsx`), updated to
...
- Line highlight: ```` ```text {3,7-9} ```` highlights with
...
- `web/src/app/not-found.tsx` — centered: `404` in 72px Geist
...
- Create Vercel project `ibex-harness-docs`, root directory `web`.
- Set build command `pnpm --filter docs build`, install command
  `pnpm install --frozen-lockfile`.
- Env: `NEXT_PUBLIC_SITE_URL=https://ibexharness.com/docs`.
...
on:
  pull_request:
    paths: ["web/**", "packages/**", "pnpm-lock.yaml"]
jobs:
  build:
...
- [ ] CI runs `playwright test` on PRs that touch `web/`.
...
- Landing site nav contains a `ibexharness.com` external link.
...
4. **OG cards** — both sites must produce valid OG cards. Visiting
   `ibexharness.com/` and `ibexharness.com/docs/getting-started/introduction` in a Slack unfurl
...
> Bootstrap a Fumadocs application at `web/` per the spec in
...
> Apply the Matte Graphite design tokens to `web/`. Copy
...
> `web/src/components/mdx/`. Each component is a named export.
> Use `lucide-react` icons with `strokeWidth=&#123;1.5}`. Register all of
> them in `web/src/mdx-components.tsx`. Create one demo page at
...
> `web/src/app/docs/[[...slug]]/opengraph-image.tsx` per
...
> Rewrite `web/src/app/api/search/route.ts` to pass each page's
...
> Create `web/src/app/not-found.tsx` matching the spec in
...
> `web/content/docs/` exactly as listed in §D.3.1. Each leaf
...
> Add Playwright. Create `web/tests/visual.spec.ts` from §D.5.1.
...
- [ ] `NEXT_PUBLIC_SITE_URL` set to `https://ibexharness.com/docs`
      on the docs Vercel project.
- [ ] Final `verify_phase15.sh` with
      `IBEX_DOCS_URL=https://ibexharness.com/docs` exits 0.
- [ ] Slack unfurl test: `ibexharness.com/docs/getting-started/quickstart`
      shows the custom OG card.
- [ ] Lighthouse on production URL: Perf ≥ 90, A11y = 100,
      Best-Practices ≥ 95, SEO = 100.
- [x] `web/content/roadmap/current-state` updated: Phase 1.5 shipped, Phase 2 may begin
