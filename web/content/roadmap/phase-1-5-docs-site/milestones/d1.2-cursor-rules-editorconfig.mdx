---
title: "d1.2 Cursor Rules"
description: "AI-assisted development on the docs site without explicit design constraints produces generic UI (gradients, blur, pill buttons) that violates the Matte Graphite system. Cursor project rules must be in place before any Fumadocs UI code lands so every edit session inherits the non-negotiable visual rules from MASTER"
summary: "AI-assisted development on the docs site without explicit design constraints produces generic UI (gradients, blur, pill buttons) that violates the Matte Graphite system. Cursor project rules must be in place before any Fumadocs UI code lands so every edit session inherits the non-negotiable visual rules from MASTER"
status: "completed"
milestoneId: "d1.2"
goal: "[D.1](/roadmap/phase-1-5-docs-site/goals)"
estimatedEffort: "30 minutes"
phase: "phase-1-5-docs-site"
fullTitle: "Milestone D.1.2 — Cursor rules + EditorConfig"
---
...
Spec detail: [MASTER_BRIEF.md](/roadmap/phase-1-5-docs-site/master-brief) Part D (D.1.2).
...
- `AGENTS.md` / docs-site design brief (MASTER_BRIEF §0.1 content)
- `.editorconfig` — 2-space, LF, trim trailing whitespace
- `.nvmrc` — `20.18.0`
- `web/CONTRIBUTING.md` — branch naming, anti-patterns, `pnpm docs:dev`
...
| --- | --- | --- |
| `AGENTS.md` / docs-site design brief | Add | Matte Graphite Cursor rules |
| `.editorconfig` | Add | Editor formatting |
| `.nvmrc` | Add | Node version pin |
| `web/CONTRIBUTING.md` | Add | Contributor guide for docs app |

---

## Testing Requirements

- **`verifyCursorRules_ActiveForAppsDocs`**: Editing `web/**` shows docs-site rule active
...
- [ ] Cursor status shows project rules active under `web/`
- [ ] `.editorconfig` present at repo root
- [ ] `web/CONTRIBUTING.md` documents no-gradient / no-blur rules
...
| --- | --- | --- | --- |
| Rules ignored on non-`web` paths | Medium | Low | Scope globs to `web/**` only |
