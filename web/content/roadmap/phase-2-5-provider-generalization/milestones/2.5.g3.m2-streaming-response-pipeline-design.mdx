...
**ADR required:** ADR-00XX — Streaming response transformation
...
- PR title: `design(proxy): streaming response transformation ADR and prototype (m2.5.G3.M2)`
...
`web/content/docs/adr/00XX-streaming-response-transformation.mdx` capturing:
...
web/content/docs/adr/
 00XX-streaming-response-transformation.mdx # ADR
services/proxy/internal/http/
 stream_transform_prototype.go # throwaway, not wired in
 stream_transform_prototype_test.go # benchmark
```
...
- [ ] Prototype measures and records TTFB delta, backpressure effect, and per-request memory overhead
...
- [ ] Phase 3 streaming stage design is now unambiguously constrained — no surprise architectural debates when implementation begins
