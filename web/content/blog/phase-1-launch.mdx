...
<Callout type="info" title="Historical post">
  Published at Phase 1 exit (2026-06-14). Chat completions now forward in mock/live modes; treat `501 PROVIDER_NOT_CONFIGURED` as **unregistered model**, not “forwarding missing.” Current capability boundary: [current state](/roadmap/current-state).
</Callout>

Most LLM proxies start by forwarding HTTP. IBEX Harness starts by earning trust.

Phase 1 is complete. Before we call OpenAI, assemble context, or store a single memory, the platform validates every bearer token, verifies agent identity against the organization in the URL path, normalizes request bodies into a stable internal shape, and rejects traffic that fails any check. **At Phase 1 exit**, chat routes returned `501 PROVIDER_NOT_CONFIGURED` on purpose — that response meant auth, agent verification, and routing succeeded while provider forwarding was still ahead. Phase 2 later added mock/live OpenAI-compatible forwarding.
...
## What a successful request looked like at Phase 1 exit

Follow the [Quickstart](/docs/getting-started/quickstart) for today’s path. **At Phase 1 exit**, a successful chat POST stopped at the empty provider slot and returned **`501 PROVIDER_NOT_CONFIGURED`**. That confirmed:

1. Bearer token validated over gRPC
2. Agent identity matched the token org
3. Rate limiting and body parsing ran
4. The handler reached the provider slot before any upstream LLM call

**Today (Phase 2 shipped):** registered models return HTTP **200** under default non-production `IBEX_LLM_MODE=mock`, or live OpenAI-compatible JSON/SSE when `IBEX_LLM_MODE=live` and `OPENAI_API_KEY` are set. `501 PROVIDER_NOT_CONFIGURED` now means the **model id is not registered** — see [Chat completions](/docs/api-reference/chat-completions) and [current state](/roadmap/current-state).

<Callout type="warning" title="Honest scope (still true)">
  Memory writes, context assembly, and the operator dashboard remain Phase 3+. Live inventory: [current state](/roadmap/current-state).
</Callout>
...
| [Phase 2](/roadmap/phase-2-single-provider) | Single provider E2E — **shipped** (mock/live chat, cache, directives, sessions, traces) |
...
<CardGrid>
  ...
  <DocCard title="Chat completions API" href="/docs/api-reference/chat-completions" description="Current proxy route shape, mock/live modes, and errors." iconName="Waypoints" category="reference" />
</CardGrid>
