...
<Callout type="warning" title="Dirty migration 008 / 012">
  Stale dev data can leave invalid `tokens.revoked_by` FKs or cross-org `agent_id` / `user_id` binds. Reset with `make compose-dev-reset && make db-migrate`, or repair with `make db-repair-token-fks`.
</Callout>
