...
<Steps>
  ...
  <Step title="Rate limit">
    Org-level RPM via shared Redis. An exhausted quota returns 429; Redis dependency failure returns 503 and does not proceed to a provider, per [ADR-0081](/docs/adr/0081-fail-closed-proxy-runtime-controls).
  </Step>
</Steps>
