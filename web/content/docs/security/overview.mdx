...
description: Security objectives S1–S6, threat model summary, and validated auth + proxy invariants.
audience: operator
scope: shipped
verifiedDate: "2026-08-15"
sourceOfTruth: web/engineering/SECURITY.md; services/proxy security-integration tests
---

IBEX Harness sits on a high-value data path — LLM traffic, persistent memory, org-scoped tokens, and billing events. Security is an invariant, not a phase. This page summarizes what must always be true and what we defend against on the shipped auth + proxy path.

<Callout type="note" title="Shipped surface">
  Auth, agent identity, rate limits, error envelopes, and **mock-provider** forwarding are exercised by the `security-integration` CI job. Live OpenAI/OpenRouter forwarding is covered by operator smokes (`make dev-smoke-live`), not that job. Memory injection defenses and dashboard MFA ship in Phase 3+.
</Callout>
...
| Proxy Redis unavailable (rate limit) | **Fail closed** — return 503 `SERVICE_DEGRADED`; do not reach provider work ([ADR-0081](/docs/adr/0081-fail-closed-proxy-runtime-controls)) |
