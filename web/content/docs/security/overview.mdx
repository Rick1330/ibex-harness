...
<Callout type="note" title="Shipped surface">
  Auth, agent identity, rate limits, error envelopes, and provider forwarding (mock/live) are exercised by the `security-integration` CI job. Memory injection defenses and dashboard MFA ship in Phase 3+.
</Callout>
