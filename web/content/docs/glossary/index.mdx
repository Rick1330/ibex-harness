...
<h3 id="fail-open">Fail open</h3>

Continue the protected operation when a dependency cannot make a reliable decision. The current Proxy does **not** fail open on shared rate-limit or organization model-policy dependency failures; those return 503. Its explicit local-development Noop limiter is not a production fallback. See [fail-closed Proxy controls](/docs/adr/0081-fail-closed-proxy-runtime-controls).
