---
title: Overview
description: High-level system diagram, design philosophy, and honest Phase 2 shipped scope for IBEX Harness.
audience: integrator
scope: shipped
...
IBEX Harness is a distributed platform for an authenticated LLM proxy today, with persistent agent memory and context assembly planned for Phase 3. Agent applications call a low-latency [LLM proxy](/docs/proxy/overview); the proxy authenticates every request through the [auth service](/docs/auth/overview), applies org directives and session context, then forwards to a mock or live provider.

<Callout type="info" title="Phase 2 shipped — honest scope">
...
<FeatureGrid
  features={[
    ...
    },
    {
      ...
      description:
        "Auth unreachable → fail closed (503). Proxy Redis rate-limit failure → fail closed (503); auth cache skips wrap. ClickHouse down → traces drop, chat continues.",
    },
    {
      icon: Brain,
