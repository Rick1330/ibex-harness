...
Provider adapters translate normalized IBEX chat requests into upstream LLM API calls and stream responses back to clients. The registry ships with a **mock** adapter (default), an **OpenAI-compatible** adapter, and an **Anthropic** adapter ([ADR-0040](/docs/adr/0040-anthropic-provider-adapter)).

<Callout type="info" title="Modes">
  `IBEX_LLM_MODE=mock` (default) returns HTTP **200** from an in-process stub for registered models.
  Set `IBEX_LLM_MODE=live` with `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` to register those vendors.
  Live mode requires at least one key. Unknown models still return `501 PROVIDER_NOT_CONFIGURED`. Design:
  [ADR-0025](/docs/adr/0025-llm-provider-abstraction), [ADR-0026](/docs/adr/0026-openai-client-design),
  [ADR-0027](/docs/adr/0027-streaming-dual-write), [ADR-0040](/docs/adr/0040-anthropic-provider-adapter).
</Callout>
  ...
  steps={[
    {
      ...
      description:
        'Each configured adapter is registered into provider.Registry by SupportedModels(). Live mode registers OpenAI and/or Anthropic when keys are present.',
    },
    {
    ...
    },
    {
      title: 'Call upstream',
      description:
        'Adapter applies provider credentials from process env (never from the client request body). Anthropic uses x-api-key + anthropic-version.',
    },
    {
