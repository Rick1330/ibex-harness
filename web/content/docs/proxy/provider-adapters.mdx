...
<Callout type="info" title="Phase 2 progress">
  Milestones [2.1.1](/roadmap/phase-2-single-provider/milestones/2.1.1-provider-interface-and-registry) through
  [2.1.5](/roadmap/phase-2-single-provider/milestones/2.1.5-provider-error-mapping) ship `packages/provider`,
  the OpenAI adapter, SSE dual-write forwarding, provider routing, and centralized error mapping per
  [ADR-0025](/docs/adr/0025-llm-provider-abstraction),
  [ADR-0026](/docs/adr/0026-openai-client-design), and [ADR-0027](/docs/adr/0027-streaming-dual-write).
  Set `IBEX_LLM_MODE=live` and `OPENAI_API_KEY` to forward; default `mock` returns `501 PROVIDER_NOT_CONFIGURED`.
</Callout>
