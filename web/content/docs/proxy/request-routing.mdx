...
<Endpoint
  ...
  description="OpenAI-compatible chat completions. Requires ProxyChatCompletion permission. Mock mode returns 200; unknown models return 501."
/>
...
flowchart LR
  ...
  AU --> AG[agentVerify]
  AG --> RL[rateLimit]
  RL --> DR[directiveResolve]
  DR --> CP[chatParse]
  CP --> PR[providerRouter]
  PR --> H[handler]
```

Inside the chat handler (not a middleware hop): when `IBEX_CONTEXT_ENABLED` and a context client are present, **Assemble** runs via gRPC, then messages are injected and forwarded to the provider. Failures fail open to Phase 2 directive-only injection.

Terminate is a separate authenticated route:

```mermaid
flowchart LR
  T[POST terminate] --> TC[Complete session]
  TC -->|status completed| EN[async HTTP enqueue ADR-0072]
```
...
See [Provider adapters](/docs/proxy/provider-adapters) for mock vs live configuration and [roadmap](/roadmap/phase-2-single-provider) for remaining exit-gate work.
