...
| MinIO | `cgr.dev/chainguard/minio:latest` | **9000** (API), **9001** (console) | Object storage (Phase 2+) |
...
<ProcessSteps
  steps={[
    ...
    },
    {
      title: 'Smoke test',
      description:
        'make dev-smoke checks /health, /ready, auth failures, and chat HTTP 200 for registered gpt-4o. Expect the default non-production IBEX_LLM_MODE=mock (or a live proxy that also returns 200); use make dev-smoke-live for explicit live OpenRouter checks.',
    },
  ]}
/>
