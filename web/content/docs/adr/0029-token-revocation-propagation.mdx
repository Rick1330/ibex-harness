...
3. **Auth publisher:** After durable Postgres revoke, publish asynchronously (`REDIS_URL`). Redis failure must **not** fail `RevokeToken` — Postgres remains source of truth; LRU will expire naturally within 30s.
4. **Proxy subscriber:** On message, call `CachingValidator.InvalidateByTokenID(tokenID)`. The claims LRU maintains a secondary `tokenID → digest` index populated on `putLRU`. Lifecycle owned by `packages/shutdown`.
