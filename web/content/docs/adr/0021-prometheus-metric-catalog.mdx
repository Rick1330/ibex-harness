---
title: "ADR-0021: Prometheus Metric Catalog (Phase 1)"
description: "Architecture decision record 0021."
adrId: "0021"
status: "Accepted"
date: "2026-06-07"
---
...
**Status:** Accepted  
**Date:** 2026-06-07  
**Milestone:** [1.3.2](/roadmap/phase-1-core-platform/milestones/1.3.2-prometheus-metric-catalog)

## Context

Auth and proxy exposed `/metrics` via duplicate custom mutex-based implementations with inconsistent metric names, histogram buckets, and high-cardinality `path` labels (raw URLs with UUIDs). Repository guidance (`AGENTS.md`) requires a shared `packages/metrics` registry.

M1.3.1 initialized OTel tracing ([ADR-0019](/docs/adr/0019-opentelemetry-provider-configuration)). Phase 1 Prometheus exposition uses `prometheus/client_golang` directly — not the OTel meter bridge.
...
| `ibex_proxy_idempotency_total` | Counter | `result` (`hit`/`miss`/`conflict`/`in_progress`/`redis_error`/`skipped`) | proxy |
| `ibex_proxy_idempotency_duration_seconds` | Histogram | — | proxy |
| `ibex_proxy_response_pipeline_stage_duration_seconds` | Histogram | `stage`, `result` | proxy |
| `ibex_proxy_response_pipeline_stage_fail_open_total` | Counter | `stage` | proxy |
| `ibex_proxy_context_assemble_fallback_total` | Counter | `reason` (gRPC code name or `nil_response`) | proxy |
...
`ibex_auth_validate_token_duration_seconds` and `ibex_auth_validate_agent_duration_seconds` are recorded on the **auth gRPC server**. Proxy-side `ibex_proxy_auth_validate_*` and `ibex_proxy_agent_validate_*` metrics are **retired** (supersedes [ADR-0011](/docs/adr/0011-proxy-auth-client) §8 proxy metric names).

Rate-limit metrics deferred in [ADR-0015](/docs/adr/0015-proxy-rate-limit-skeleton) §7 are implemented in M1.3.2.
...
- [Milestone 1.3.2](/roadmap/phase-1-core-platform/milestones/1.3.2-prometheus-metric-catalog)
- [ADR-0019](/docs/adr/0019-opentelemetry-provider-configuration)
