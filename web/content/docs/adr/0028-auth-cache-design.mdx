...
- `packages/authcache` owns `Validator`, `Result`, `CachingValidator`, `TokenHash`, `Invalidate`.
- Invalid-token bloom uses an `RWMutex` and **two-generation rotation** when adds reach `BloomExpectedItems`, keeping the FP rate bounded on long-lived processes.
