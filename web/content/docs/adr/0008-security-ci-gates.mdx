...
| `benchmarks` | `benchmarks/**`, `web/public/benchmarks/**` | `benchmark.yml` workflow paths only |
...
| Dependabot `go.mod` PR | Go + security | No `web-build` unless lockfile also changes |
