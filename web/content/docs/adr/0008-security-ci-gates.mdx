...
CI uses `go-version-file: go.mod` so the runner Go version tracks `go.mod` (currently **1.25.12**; `golang.org/x/crypto` ≥ v0.54.0; golangci-lint **v2.4+** for Go 1.25). OSV scans use `--call-analysis=go` alongside a dedicated `govulncheck` job for reachable Go vulnerabilities.
