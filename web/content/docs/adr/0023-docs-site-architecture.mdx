...
- Credentials in GitHub Environment **`production`** (`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`); see `ibex-harness-workspace/ops/cloudflare/README.md` (local-only operator runbook).
...
- **Mixed model (current):** manual behavioral MDX + proto-backed gRPC pages + HTTP route inventory from the Go router.
- OpenAPI-driven pages (`fumadocs-openapi`) remain deferred until `services/proxy` publishes a maintained `openapi.yaml` that is CI-validated against implemented routes.
- **Amendment (2026-08-15):** Documented inventory at `/docs/api-reference/http-routes`; search indexes MDX headings / structured excerpts in addition to description.
