...
Every model ID returned by `SupportedModels()` must resolve in `catalog`. Missing entries return `ErrMissingCapability` at startup — never a silent miss at request time. Duplicate model IDs still return `ErrDuplicateModel`.

### 4) ExtraModels require explicit overlays

`IBEX_LLM_EXTRA_MODELS` / `ANTHROPIC_EXTRA_MODELS` remain allowlist extensions. Operators must also supply capability overlays (`IBEX_MODEL_CAPABILITY_OVERLAYS` JSON) for every extra model ID. Overlays merge on top of the built-in catalog (overlay wins on ID collision). Extra models without overlay entries fail registry construction.
