...
> **Current runtime policy:** The Phase 1 failure decision below is historical and superseded for the current Proxy by [ADR-0081](/docs/adr/0081-fail-closed-proxy-runtime-controls). Redis-backed rate-limit failure now returns 503 and does not proceed to a provider.
...
- Org-level calendar-minute window: key `ratelimit:{org_id}:rpm:{unix_minute}` (org_id is the second key segment; see `AGENTS.md` multi-tenant Redis rules)
...
| Rate limit | Phase 1 historical behavior: fail **open** (superseded by [ADR-0081](/docs/adr/0081-fail-closed-proxy-runtime-controls)) |
