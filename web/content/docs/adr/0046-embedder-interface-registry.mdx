...
- Values are `float32` little-endian bytes only (never pickle). Mixed batches use one `MGET` + one pipeline `SET EX`.
- Prometheus: `ibex_embedder_cache_requests_total{backend,result}` per text; `GET /metrics`. JSON `backend` stays the inner name (not `cached:…`).
