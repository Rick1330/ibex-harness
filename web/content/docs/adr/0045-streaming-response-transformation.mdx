...
- Hard per-request buffer cap; overflow → fail-closed (`ErrPrototypeBufferOverflow` in the design prototype).
...
Environment: linux/amd64, Intel Core Ultra 7 155H. Command: `go test ./services/proxy/internal/http/ -bench BenchmarkPrototypeWindow -benchtime=100ms`.

| Measurement | Result | Phase 3 implication |
| --- | --- | --- |
| First emit CPU (holdback 16) | ~1.5 µs / first emit | Buffer CPU is negligible vs model token latency |
| First emit CPU (holdback 64) | ~6 µs | Default holdback is cheap in-process |
| First emit CPU (holdback 256) | ~45 µs | Still ≪ 1 ms; perceptual TTFT dominated by waiting for N model tokens |
| Steady `Feed` (holdback 64) | ~1.6 µs/op | Fits ADR-0034 overhead discipline for the buffer itself |
| 100 concurrent windows × 64 holdback | 6400 runes retained; ~91 KiB allocs in bench construct | Budget ~holdback runes per in-flight stream (+ small struct overhead) |
| Max `Feed` vs 50 ms backpressure threshold | max feed ≪ 50 ms (bench reports ~0.3 ms worst sample) | Holdback does **not** invent ADR-0027 backpressure; slow client flush remains the signal |

**Perceptual added TTFT** (not CPU): approximately `holdback_runes / model_tokens_per_second` plus inter-chunk jitter. Industry reports for similar windows often cite ~30–80 ms wall delay when the model is already streaming; Phase 3 must measure against real providers before setting SLOs.
