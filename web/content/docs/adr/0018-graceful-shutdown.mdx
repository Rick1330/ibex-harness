...
Ad-hoc signal handling in `main.go` is forbidden ([29-ibex-packages.mdc](/AGENTS.md)).
...
Canonical name per [24-config-management.mdc](/AGENTS.md). Documented in service `.env.example` files and [ENVIRONMENT_VARIABLES.md](/docs/environment_variables).
...
```text
SIGTERM → drain=IBEX_SHUTDOWN_TIMEOUT → http.Server.Shutdown → pub/sub stop → checkpoint pool → session sweeper → ClickHouse writer drain → auth gRPC Close → Redis Close → Postgres Close → exit

SIGINT  → drain=0 (immediate) → same handler order, best-effort only → exit
         (does not wait for ClickHouse writer drain / other handlers to finish)
```

**Auth:**

```text
SIGTERM → drain=IBEX_SHUTDOWN_TIMEOUT → gRPC GracefulStop (Stop fallback on timeout) → http.Server.Shutdown → db.Close → exit
...
- OTel `Providers.Shutdown` registration (M1.3.1) — **done**
- ClickHouse writer flush (Phase 2) — **done in [2.5.2](/roadmap/phase-2-single-provider/milestones/2.5.2-clickhouse-client)** (`packages/clickhouse.Writer` registered on proxy shutdown)
...
- [29-ibex-packages.mdc](/AGENTS.md)