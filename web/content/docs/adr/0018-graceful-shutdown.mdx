...
```text
SIGTERM/SIGINT → http.Server.Shutdown → pub/sub stop → checkpoint pool → session sweeper → ClickHouse writer drain → auth gRPC Close → Redis Close → Postgres Close → exit
```
