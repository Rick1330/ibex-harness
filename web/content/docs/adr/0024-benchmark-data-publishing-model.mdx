...
- Harness `notify-benchmark-bot` / `notify-hnsw-benchmark-bot` jobs send
  `repository_dispatch` when `BENCHMARK_BOT_ENABLED=true` on **schedule**,
  **workflow_dispatch**, or path-triggered **push** to `main` (so published
  history is not Sunday-only).
- Interim `open-benchmark-data-pr` removed; bot opens data PRs with App identity.
- Proxy and Memory HNSW keep separate JSON artifacts/files, but publish onto **one**
  shared branch/PR (`chore/bench-data-publish`) so reviewers merge a single data PR.
...
- Post-merge benchmark data PRs are opened by ibexharness-benchmark-bot (one shared PR
  for pending suite files; configure per bot repo APP_SETUP.md).
