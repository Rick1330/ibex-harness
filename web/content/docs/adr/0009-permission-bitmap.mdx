---
title: "ADR-0009: Permission bitmap layout"
description: "Architecture decision record 0009."
adrId: "0009"
status: "Accepted"
date: "2026-06-04"
...
- **Helpers:** `Has`, `HasAny`, `RequiresStepUp` (canonical; `RequiresMFA` is an alias), `UsesReservedHighBits`
...
| 19 | Session / Operator | `OperatorMetadataRead` (4.P.1) |
| 20-23 | Session | Reserved |
| 24 | Trace | `TraceRead` |
...
| 38 | Admin / Operator | `OperatorRedactedRead` (4.P.1) |
| 39 | Admin / Operator | `OperatorRawRead` (step-up; 4.P.1) |
| 40 | Marketplace | `MarketplacePublish` |
| 41 | Marketplace | `MarketplaceInstall` |
| 42 | Operator | `OperatorExport` (step-up; 4.P.1) |
| 43 | Operator | `OperatorDelete` (step-up; 4.P.1) |
| 44 | Operator | `OperatorReplay` (step-up; 4.P.1) |
| 45 | Operator | `SecretUse` (step-up; proxy credential get; 4.P.1) |
| 46 | Operator | `PolicyChange` (step-up; 4.P.1) |
| 47 | Operator | `BreakGlass` (step-up; 4.P.1) |
| 48 | Federation | `FederationShare` |
| 49 | Federation / Governance | `LegalHoldManage` (step-up; 4.P.3) |
| 50-55 | Federation | Reserved |
| 56-63 | — | Reserved (do not use in v1) |

### 2b) 4.P.1 amendment

- Helpers: `RequiresStepUp` (canonical); `RequiresMFA` remains an alias.
- Role defaults: `ViewerOperatorDefault` / `MemberOperatorDefault` = metadata+redacted; `AdminOperatorDefault` adds export+policy-change **without** raw/secret-use/break-glass.
- `Admin` management set still excludes operator raw/secret/break-glass.

### 2c) 4.P.3 amendment

- `LegalHoldManage` (bit 49) requires step-up; included in `AdminOperatorDefault` so owner/admin role bitmaps can set/clear holds.
...
- [ARCHITECTURE.md](/docs/architecture) — auth permission table
- [Milestone 1.1.5](/roadmap/phase-1-core-platform/milestones/1.1.5-permission-bitmap-contract-and-adr)
- [ADR-0006](/docs/adr/0006-auth-proto-contract) — `ValidateTokenResponse.permissions`