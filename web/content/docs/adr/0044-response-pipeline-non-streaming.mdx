...
- **Fail-open:** non-critical stage errors log (stage name + error class), restore a pre-stage snapshot (so in-place mutations from the failing stage are discarded), and return the last good response.
- **Fail-closed:** stages implementing `SecurityCritical` with `SecurityCritical() == true` propagate errors to the proxy (mapped to 502/503 envelope).
