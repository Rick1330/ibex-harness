...
7. **Fail-open:** Redis errors or claim/commit timeout (`IBEX_IDEMPOTENCY_REDIS_TIMEOUT`, default **50ms**) proceed without dedupe (WARN + `redis_error` metric). This is a deliberate idempotency/billing trade-off only; the separate organization rate-limit control fails closed under [ADR-0081](/docs/adr/0081-fail-closed-proxy-runtime-controls).
