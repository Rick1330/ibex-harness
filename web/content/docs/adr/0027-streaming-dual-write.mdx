...
Parse errors in the accumulator must never fail `Write` or block the forward path. Incomplete accumulation logs a warning and continues; the client still receives bytes. Prefer a missing trace over delayed TTFB.

Soft content cap: **1 MiB** of accumulated completion text. Beyond the cap, further content is dropped from the accumulator (forward still receives full upstream bytes).
...
Backpressure: increment when a client write returns a short write or takes longer than 50ms for an event (bounded signal; not per-token).
...
- Milestone 2.1.5: centralized `MapProviderError` for pre-stream failures
