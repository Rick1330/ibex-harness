...
5. **Tenant isolation:** Both tables have `org_id`, `ENABLE` + `FORCE ROW LEVEL SECURITY`, NULLIF-safe org setting, and service-account bypass — matching the agents pattern. Ownership is structural via composite FKs: `(agent_id, org_id)`, `(directive_id, org_id)`, and `active_version_id` tied to both `(id, org_id)` and `(id, directive_id)` on versions.
6. **Append-only versions:** `ibex_app` has no `UPDATE` on `directive_versions`; a BEFORE UPDATE trigger rejects mutations. `DELETE` remains for parent CASCADE cleanup.
7. **Deferred circular FK:** Active-version FKs are added `NOT VALID` then `VALIDATE` after `directive_versions` exists.
