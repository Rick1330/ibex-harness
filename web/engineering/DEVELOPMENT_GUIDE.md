...
When implementing a milestone from [`web/content/roadmap/`](../content/roadmap/) (public `/roadmap`), use this pattern until a GitHub issue exists:
...
One milestone per branch (see §6.2). Branch names and PR titles for each milestone are listed in the milestone file under `web/content/roadmap/`.
...
4. **CURRENT_STATE:** Open a small docs PR (e.g. `docs/current-state-m1-x-x`) updating [`web/content/roadmap/current-state.mdx`](../content/roadmap/current-state.mdx) with the merge SHA — do not fold unrelated product code into that PR.
...
- **Complexity:** `funlen` / `gocognit` in `.golangci.yml` with `issues.new` + `new-from-rev: origin/main` + `whole-files: true` (unchanged hotspots grandfathered; any edit to a file re-checks the whole file).
...
- `web/engineering/ARCHITECTURE.md`
- `web/engineering/API_DOCUMENTATION.md`
- `web/engineering/DATABASE_SCHEMA.md`
- `web/engineering/SECURITY.md`
- `web/engineering/TESTING_STRATEGY.md`
- `web/engineering/CODING_STANDARDS.md`
...
…requires an ADR in [`web/content/docs/adr/`](../content/docs/adr/) (public `/docs/adr`).
...
AI sessions lose context. Keep a **session workspace** next to the git clone (never committed). The versioned roadmap in `web/content/roadmap/` describes *what to build*; the workspace holds *session continuity*, *archived audits*, and *execution prompts*.
...
ibex-harness-workspace/
  current_state.md          # session scratch; pointers to web/content/roadmap/current-state.mdx
  decisions.md
  active_task.md
  handoff.md
  known_issues.md
...
1. Update [`web/content/roadmap/current-state.mdx`](../content/roadmap/current-state.mdx) in the repo (via PR).
2. Refresh workspace `current_state.md` and `handoff.md`.
3. Log surprises in [`web/content/roadmap/findings.mdx`](../content/roadmap/findings.mdx) when the plan changes.
...
Canonical status: `web/content/roadmap/current-state.mdx` in the repo (public `/roadmap/current-state`).
...
## REST
- web/engineering/API_DOCUMENTATION.md sections updated

## DB
- web/engineering/DATABASE_SCHEMA.md updated
