...
| `POSTGRES_DSN` | Conditional | (empty) | Postgres for directive reads and session store writes; empty → session store disabled; with Redis enables cached directive resolver | Secret; must match migrated schema |
