...
**None found.** `memory-test`, `memory-integration`, and `memory-security-integration` are required on both `ci-gate-python` and `ci-gate-go`; `e2e-smoke-p3-memory` is required on `ci-gate-python` only. Published benchmark JSON on `main` (2026-08-31) reports `status: pass` for HNSW, ranking-quality, and write-pipeline suites.
...
| 5 | All 12 ISO-* security cases pass | `memory-security-integration-test-ci.sh` requires ≥12 `test_memory_iso_*`; includes ISO-1.4 relationships cross-org | **Pass** |
