...
**Stage status:** PR #897 and PR #900 are merged on `main` at the current re-baseline (`df657fd` and `0f087d2`, respectively). Their local/source contracts are present, but hosted CI, live branch-protection parity, canonical runtime, and staging evidence remain open. No P0/P1/P6, D0/D1, Track D, or Phase 4 closure is claimed by this plan.
...
**Current-stage acceptance:** The merged PR-1 and PR-2 evidence reports record passing local/source suites, while external-service integration, hosted required CI, live branch-protection parity, staging Secret delivery/health, and independent review remain separate gates. PR 3 is the next implementation stage and must preserve the distinction between source, contract, runtime, and staging evidence.

**Current PR-1/PR-2 technical scope status:** PR #897 provides the merged Proxy fail-closed controls, explicit profile/Redis requirements, CI aggregate declarations, and source-level regression guards. PR #900 provides AuthService-owned RS256 session lifecycle pieces, route-policy parity, CSRF/security headers, and local authorization/session coverage. The full external-service integration run, live branch-protection update, staging Redis Secret delivery/health, hosted CI, browser behavior, and final independent code review remain open gates. PR #900's legal-hold/high-impact browser path is explicitly recorded in `web/engineering/pr3-evidence/p1-route-policy-matrix.md` and must not be treated as operator-session closure without a contract decision.
