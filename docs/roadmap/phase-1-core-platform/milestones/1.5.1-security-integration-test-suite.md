...
**Status:** Complete
**Goal:** [1.5 — Phase 1 Security Gate](../goals.md#goal-15-phase-1-security-gate)
...
- [1.1.7](./1.1.7-users-and-agents-schema.md) — users + agents tables
- [1.2.4](./1.2.4-proxy-rate-limit-skeleton.md) — rate limiting middleware
- [1.2.5](./1.2.5-proxy-agent-identity-verification.md) — agent identity verification
- [1.2.6](./1.2.6-and-1.2.7-reqid-and-shutdown.md) — request ID middleware
- [1.4.1](./1.4.1-developer-experience-baseline.md) — seed data and compose-test stack
...
| SEC-1.2 | `Authorization: Bearer` with empty value | `""` | 401 |
| SEC-1.3 | Malformed token (not parseable) | `"not_a_token"` | 401 |
| SEC-1.4 | Valid format, token not in DB | `"ibex_sk_unknowntoken"` | 401 |
| SEC-1.5 | Valid token, revoked | Revoke via API, then use | 401 within 100ms of revocation |
...
| SEC-3.4 | Response to SEC-3.2 does NOT differ in timing from SEC-3.3 | p95 timing delta < 25ms (CI-safe; milestone target 5ms local) |
...
- [x] All 31 test cases in the matrix above pass with `-tags=integration`
- [x] SEC-3.4: p95 timing delta between cross-org denials < 25ms in CI
...
- [x] `docs/SECURITY.md` updated with the security model that this suite validates
