...
Spec detail: [MASTER_BRIEF.md](../MASTER_BRIEF.md) Part D (D.1.1).
...
Without pnpm workspaces and Turborepo, we cannot add `apps/docs/` in a way that matches the master brief, caches `next build`, and exposes `pnpm docs:dev` / `pnpm docs:build` from the repo root.
...
- Creating `apps/docs/` application code (D.2.1)
- npm or yarn lockfiles
- Moving Go services into `apps/`
...
`chore(docs): pnpm workspace and Turborepo for apps/docs (D.1.1)`

---

## Prerequisites

- Wave 0 roadmap scaffold merged (repo-guards allows `apps/`, ADR-0023 accepted)
