...
| Docs + Go push to `main` | `changes.docs=true`, `changes.go=true` | Full Go gate + `docs-build` + `auth-validate-smoke` + `proxy-agent-verify-smoke` + `govulncheck` |
...
All CI jobs that run `go test` emit JUnit XML via [gotestsum](https://github.com/gotesttools/gotestsum) (pinned `v1.13.0`) and upload results to [Trunk Flaky Tests](https://app.trunk.io) (org: `ibexharness`). Uploads are **informational** (`continue-on-error: true`); they do not block merges.
