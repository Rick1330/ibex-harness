...
| `license-check` | `license-check` | Dependency license allowlist | Medium | MIT-compatible licenses only |
...
| Docs + Go push to `main` | `changes.docs=true`, `changes.go=true` | Full Go gate + `docs-build` |
...
- Docker image publish runs on `v*.*.*` tags via `release.yml` → `docker-publish.yml`

## Trunk Flaky Tests (JUnit uploads)

All CI jobs that run `go test` emit JUnit XML via [gotestsum](https://github.com/gotesttools/gotestsum) (pinned `v1.12.0`) and upload results to [Trunk Flaky Tests](https://app.trunk.io) (org: `ibexharness`). Uploads are **informational** (`continue-on-error: true`); they do not block merges.
